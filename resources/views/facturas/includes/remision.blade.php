...
@foreach($itemsRemision as $itemR)
    ...
    <tr id="{{$cont}}">
        ...
        <td  class="no-padding" >
            <select class="form-control form-control-sm selectpicker no-padding"  title="Seleccione" data-live-search="true" data-size="5" name="item[]" id="item{{$cont}}" onchange="rellenar({{$cont}},this.value);"  onload="alert('ok')" required="">
                @foreach($inventario as $item)
                        <option value="{{$item->id}}"  {{$itemR->producto==$item->id?'selected':''}}>{{$item->producto}} - ({{$item->ref}})</option>
                @endforeach
            </select>
        </td>
        <td>
        ...
        <td>
            <select class="form-control form-control-sm selectpicker" name="impuesto[]" id="impuesto{{$cont}}" title="Impuesto" onchange="totalall();" required="">
                @foreach($impuestos as $impuesto)
                    <option value="{{$impuesto->id}}" porc="{{$impuesto->porcentaje}}"   {{$itemR->id_impuesto==$impuesto->id?'selected':''}}>{{$impuesto->nombre}} - {{$impuesto->porcentaje}}%</option>
                @endforeach
            </select>
        </td>
        ...
        <td>
            <input type="number" class="form-control form-control-sm" id="cant{{$cont}}" name="cant[]"  value="{{$itemR->cant}}" placeholder="Cantidad" onchange="total({{$cont}});" min="1"  >
            <p class="text-danger nomargin" id="pcant{{$cont}}"></p>
        </td>
        <td>
            <input type="text" class="form-control form-control-sm text-right" id="total{{$cont}}" value="{{App\Funcion::Parsear($itemR->total())}}" disabled="" >
        </td>
        <td>

...
@section('boton')
    <a href="{{ route('facturas.show',$factura->id )}}"  class="btn btn-danger btn-sm" title="Regresar"><i class="fas fa-step-backward"></i></i> Regresar</a>
...
<script>
    ...
    function() {
        ...
        if (window.location.pathname.split("/")[1] === "software") {
            var url = '/software/empresa/facturas/logs/{{ $factura->id }}';
        } else {
            var url = '/empresa/facturas/logs/{{ $factura->id }}';
        }
